...
def exact_keys(value: Any, expected: set[str], where: str) -> dict[str, Any]:
    ...
    if actual != expected:
        ...
        raise VerificationError(f"{where} keys differ; missing={missing}, extra={extra}")
    return value
...
def require_hash(value: Any, where: str, pattern: re.Pattern[str] = HEX64) -> str:
...
def _manifest_entries(snapshot: Path, manifest: dict[str, Any]) -> None:
    ...
    actual_manifest = canonical_sha256(manifest, omit="manifest_sha256")
    ...
    for index, item in enumerate(files):
        entry = exact_keys(item, {"path", "bytes", "sha256"}, f"manifest.files[{index}]")
        ...
        if path.is_symlink() or not path.is_file():
            raise VerificationError(f"manifested file is missing, non-file, or symlink: {relative}")
    ...
    if declared != SNAPSHOT_FILES or actual_files != SNAPSHOT_FILES:
        raise VerificationError(
            f"snapshot file set differs; declared={sorted(declared)}, actual={sorted(actual_files)}"
        )
...
def _source_shape(source: dict[str, Any]) -> None:
    exact_keys(
        source,
        {"schema_version", "snapshot_id", "scenario_id", "phase", "scenario_sha256", "source"},
        "source summary",
    )
    require_string(source["schema_version"], "source.schema_version", "xcp-research-source-summary-v1")
    ...
    detail = exact_keys(
        source["source"],
        {"project", "authorization_basis", "revision", "observation_status", "trace_sha256"},
        "source.source",
    )
    require_string(detail["project"], "source.source.project")
    require_string(detail["authorization_basis"], "source.source.authorization_basis", "open-source license")
    require_hash(detail["revision"], "source.source.revision", HEX40)
    require_string(detail["observation_status"], "source.source.observation_status", "complete_for_scenario")
...
def _target_shape(target: dict[str, Any]) -> None:
    exact_keys(
        target,
        {"schema_version", "snapshot_id", "scenario_id", "phase", "scenario_sha256", "source_trace_sha256", "target"},
        "target summary",
    )
    require_string(target["schema_version"], "target.schema_version", "xcp-research-target-summary-v1")
    ...
    detail = exact_keys(
        target["target"],
        {"class", "worker_artifact_sha256", "project_sha256", "execution_status", "lifecycle_operations", "lifecycle_receipt_sha256", "observation_evidence_sha256", "trace_sha256"},
        "target.target",
    )
    require_string(detail["class"], "target.target.class", "constrained Xbox Series X public-UWP candidate")
    for field in ("worker_artifact_sha256", "project_sha256", "lifecycle_receipt_sha256", "observation_evidence_sha256", "trace_sha256"):
        require_hash(detail[field], f"target.target.{field}")
    require_string(detail["execution_status"], "target.target.execution_status", "complete_for_scenario")
    operations = exact_keys(detail["lifecycle_operations"], {"passed", "total"}, "target.target.lifecycle_operations")
...
def _differential_shape(report: dict[str, Any]) -> None:
    exact_keys(
        report,
        {"schema_version", "snapshot_id", "scenario_id", "phase", "identities", "measurement", "interpretation", "coverage", "decision", "historical_note"},
        "differential report",
    )
    require_string(report["schema_version"], "differential.schema_version", "xcp-research-differential-report-v1")
    ...
    identities = exact_keys(
        report["identities"],
        {"scenario_sha256", "source_trace_sha256", "target_trace_sha256", "historical_report_sha256", "root_cause_evidence_sha256"},
    ...
    measurement = exact_keys(
        report["measurement"],
        {"category", "published_decimal", "canonical_artifact_decimal", "unit", "tolerance", "significant"},
    ...
    require_string(measurement["canonical_artifact_decimal"], "measurement.canonical_artifact_decimal")
    if not isinstance(measurement["significant"], bool):
        raise VerificationError("measurement.significant must be boolean")
    tolerance = exact_keys(measurement["tolerance"], {"kind", "maximum_decimal", "within_tolerance"}, "measurement.tolerance")
    ...
    interpretation = exact_keys(report["interpretation"], {"classification", "diagnostic_gates"}, "differential.interpretation")
    require_string(interpretation["classification"], "interpretation.classification", "runtime_response_difference")
    gates = exact_keys(interpretation["diagnostic_gates"], {"passed", "total"}, "interpretation.diagnostic_gates")
    if require_count(gates["passed"], "diagnostic_gates.passed") != require_count(gates["total"], "diagnostic_gates.total", 1):
        raise VerificationError("diagnostic gates are incomplete")
    coverage = exact_keys(
        report["coverage"],
        {"scenario_hard_invariants", "scenario_perceptual_invariants", "motion_triad_hard_invariants", "motion_triad_perceptual_invariants"},
        "differential.coverage",
    )
    for name in ("scenario_hard_invariants", "scenario_perceptual_invariants"):
        row = exact_keys(coverage[name], {"passed", "evaluated"}, f"coverage.{name}")
        if require_count(row["passed"], f"coverage.{name}.passed") != require_count(row["evaluated"], f"coverage.{name}.evaluated", 1):
            raise VerificationError(f"{name} is incomplete")
    for name in ("motion_triad_hard_invariants", "motion_triad_perceptual_invariants"):
        row = exact_keys(coverage[name], {"covered", "contract"}, f"coverage.{name}")
        covered = require_count(row["covered"], f"coverage.{name}.covered")
        contract = require_count(row["contract"], f"coverage.{name}.contract", 1)
        if covered >= contract:
            raise VerificationError("Snapshot 001 must preserve incomplete global coverage")
    exact_keys(report["decision"], {"claim", "status", "global_equivalence", "reason_codes"}, "differential.decision")
    require_string(report["historical_note"], "differential.historical_note")


def _bindings(source: dict[str, Any], target: dict[str, Any], report: dict[str, Any], manifest: dict[str, Any]) -> None:
    common = (manifest["snapshot_id"], source["scenario_id"], source["phase"], source["scenario_sha256"])
    if (source["snapshot_id"], source["scenario_id"], source["phase"], source["scenario_sha256"]) != common:
        raise VerificationError("source identity is inconsistent")
    if (target["snapshot_id"], target["scenario_id"], target["phase"], target["scenario_sha256"]) != common:
        raise VerificationError("target identity is inconsistent")
    identities = report["identities"]
    if (report["snapshot_id"], report["scenario_id"], report["phase"], identities["scenario_sha256"]) != common:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("snapshot", type=Path, help="repository-local snapshot directory")
